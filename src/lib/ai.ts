...
function getMockFinderResult(req: FinderRequest): FinderResponse {
  ...
  const recommendations: FinderRecommendation[] = top5.map((channel, i) => ({
    channel,
    matchScore: Math.max(95 - i * 5 - Math.floor(Math.random() * 5), 60),
    reason: FINDER_REASONS[i % FINDER_REASONS.length],
    estimatedAdPrice: Math.round(channel.subscriberCount * 0.008 + Math.random() * 500000),
    estimatedReach: Math.round(channel.dailyAvgViews * (0.8 + Math.random() * 0.4)),
  }));

  return {
    recommendations,
    summary: `"${req.description}" 조건에 맞는 ${filtered.length}개 채널 중 상위 ${recommendations.length}개를 추천합니다. 타겟 오디언스 매칭률, 참여율, 알고리즘 스코어를 종합 분석했습니다.`,
  };
}
    ...
    const recommendations: FinderRecommendation[] = top5.map((channel, i) => {
      ...
      return {
        ...
        estimatedAdPrice:
          ...
          Math.round(channel.subscriberCount * 0.008 + Math.random() * 500000),
        estimatedReach:
          aiData?.estimatedReach ??
          Math.round(channel.dailyAvgViews * (0.8 + Math.random() * 0.4)),
      };
    });
...
function getMockContentResult(req: ContentRequest): ContentResponse {
  ...
  return {
    keywordAnalysis: {
      competitionScore: Math.floor(Math.random() * 40 + 40),
      searchVolume: ["낮음", "보통", "높음", "매우 높음"][Math.floor(Math.random() * 4)],
      relatedKeywords: [
        `${req.keyword} 추천`,
        `${req.keyword} 비교`,
        `${req.keyword} 2026`,
...
competitionScore는 0~100 사이 정수. searchVolume은 "낮음", "보통", "높음", "매우 높음" 중 하나.`;
  ...
  try {
    ...
    return {
      keywordAnalysis: {
        competitionScore: parsed.keywordAnalysis?.competitionScore ?? Math.floor(Math.random() * 40 + 40),
